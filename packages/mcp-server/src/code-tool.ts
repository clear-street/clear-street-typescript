...
const prompt = `Runs JavaScript code to interact with the Clear Street API.
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    headers: {
      ...
      'x-stainless-mcp-client-envs': JSON.stringify({
        CLEAR_STREET_BASE_URL:
          readEnv('CLEAR_STREET_BASE_URL') ?? readEnv('CLEAR_STREET_ENVIRONMENT') ?
            undefined
          : client.baseURL ?? undefined,
      }),
    },
    body: JSON.stringify({
      project_name: 'clear-street',
      code,
      intent,
      client_opts: {
        apiKey: readEnv('CLEAR_STREET_API_KEY'),
        environment: (readEnv('CLEAR_STREET_ENVIRONMENT') || undefined) as any,
      },
...
const localDenoHandler = async ({
  ...
  try {
    const sdkPkgName = '@clear-street-internal/sdk';
