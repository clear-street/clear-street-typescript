...
describe('resource events', () => {
  ...
  test.skip('getAllInstrumentEvents: request options and params are passed correctly', async () => {
    ...
    await expect(client.active.v1.instruments.events.getAllInstrumentEvents({
  ...
  test.skip('getInstrumentEvents: only required params', async () => {
    const responsePromise = client.active.v1.instruments.events.getInstrumentEvents('security_id', { security_id_source: 'CMS' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
